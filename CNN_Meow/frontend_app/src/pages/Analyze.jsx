        ...
        try {
            ...
            if (!res.ok) {
                ...
                if (detail?.error === 'not_an_animal') {
                    setError(`🐾 กรุณาอัปโหลดภาพสัตว์เท่านั้น — ระบบตรวจพบว่าภาพนี้คือ "${detail.detected_as}"`)
                } else {
                        ...
                        <motion.div key="input" initial={{ opacity: 0, y: 15 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }} className="max-w-xl mx-auto text-center">
                            ...
                            {!user ? (
                                ...
                                <div className="glass-card p-6 rounded-[3rem] border-white/10 shadow-2xl">
                                    ...
                                    {error && (
                                        <div className="mb-4 px-5 py-3 rounded-2xl bg-rose-500/10 border border-rose-500/30 text-rose-400 text-sm font-bold text-center animate-pulse">
                                            {error}
                                        </div>
                                    )}
                            ...
                            <div className="flex justify-between items-center flex-wrap gap-6 border-b border-white/5 pb-8">
                                <div>
                                    <div className="flex items-center gap-2 mb-2">
                                        ...
                                        {result.llm_used && (
                                            <div className="badge bg-amber-500/10 text-amber-400 border border-amber-500/20 px-4 py-1 rounded-full text-[17px] font-black tracking-widest uppercase">⚡ Gemini AI</div>
                                        )}
                                        {!result.llm_used && (
                                    ...
                                    <div className="flex items-center gap-5 mb-8">
                                        ...
                                        <div>
                                            ...
                                            <p className="text-[17px] text-slate-500 uppercase font-black mt-2 tracking-widest opacity-70">
                                                {plan === 'premium' ? 'Gemini-2.5-Flash // LangChain Vet Agent' : 'Fallback_Mode // CNN Basic Advice'}
                                            </p>
                                        </div>
                                    </div>
                                    ...
                                    {plan !== 'premium' && (
                                        <div className="mt-6 text-center">
                                            <button onClick={() => setPage('plans')} className="px-8 py-3 rounded-full bg-amber-500/10 border border-amber-500/30 text-amber-400 text-xm font-black uppercase tracking-widest hover:bg-amber-500/20 transition-all cursor-pointer">
                                                ⚡ อัพเกรดเป็น Premium เพื่อรับคำวิเคราะห์จาก Gemini-2.5-Flash + ระบุสายพันธุ์
                                            </button>
                                        </div>
                                    )}
                                </div>
