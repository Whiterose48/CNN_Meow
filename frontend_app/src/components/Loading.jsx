        ...
        <AnimatePresence>
            {isLoading && (
                ...
                >
                    ...
                    {/* Background Grid */}
                    <div className="absolute inset-0 z-0 scan-grid pointer-events-none opacity-50" />
                    <div className="absolute inset-0 z-0 bg-[radial-gradient(circle_at_center,transparent_0%,#010409_90%)]" />

                    {/* UI Layer */}
                    <div className="relative z-10 flex flex-col items-center justify-end h-full pb-20 w-full max-w-4xl px-6">
                        ...
                        <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 text-center pointer-events-none mt-32">
                            ...
                            >
                                <h2 className="text-3xl md:text-5xl font-black text-white tracking-[0.2em] uppercase drop-shadow-[0_0_15px_rgba(45,212,191,0.5)]">
                                    System Initializing
                                </h2>
                                <div className="flex items-center gap-3 text-teal-400 text-xs md:text-sm font-bold tracking-[0.4em] uppercase">
                        ...
                        <div className="w-full flex flex-col gap-2">
                            <div className="flex justify-between items-end text-slate-400 text-xs font-bold tracking-widest uppercase">
                                <div className="flex gap-4">
                                    <span>Core: <span className="text-white">Active</span></span>
                                    <span>Memory: <span className="text-white">Allocated</span></span>
                                </div>
                                <div className="text-teal-400 text-lg">{progress}%</div>
                            </div>

                            {/* Thin Progress Line */}
                            <div className="h-[2px] w-full bg-white/10 overflow-hidden relative">
                                <motion.div
                                    className="h-full bg-teal-400 shadow-[0_0_20px_#2dd4bf]"
                                    animate={{ width: `${progress}%` }}
                                    transition={{ ease: "linear" }}
                                />
                            </div>

                            <div className="flex justify-between text-[10px] text-slate-600 uppercase tracking-widest mt-1">
                                <span>PetInsight Protocol v2.6.4</span>
                                <span className="flex items-center gap-1"><ShieldCheck size={10} /> Verified Secure</span>
                            </div>
                        </div>
